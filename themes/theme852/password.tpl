...
{if isset($confirmation) && $confirmation == 1}
	...
	{elseif isset($confirmation) && $confirmation == 2}
		<p class="alert alert-success"><button class="close" data-dismiss="alert" type="button">×</button> {l s='A confirmation email has been sent to your address:'} {if isset($customer_email)}{$customer_email|escape:'htmlall':'UTF-8'|stripslashes}{/if}</p>
	{else}
...
<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
	<fieldset>
		...
		<div class="submit">
			<input type="submit" class="button btn btn-default" value="{l s='Retrieve Password'}" />
		</div>
	</fieldset>
</form>
{/if}
<p class="clear clearfix">
    <a class="button btn btn-default fr" href="{$link->getPageLink('authentication')|escape:'html'}" title="{l s='Back to Login'}" rel="nofollow">{l s='Back to Login'}</a> 
</p>
