...
{if isset($empty)}
	<p class="alert alert-info">{l s='Your shopping cart is empty.'}<button class="close" data-dismiss="alert" type="button">×</button></p>
{elseif $PS_CATALOG_MODE}
	<p class="alert alert-info">{l s='This store has not accepted your new order.'}<button class="close" data-dismiss="alert" type="button">×</button></p>
{else}
	...
	<p style="display:none" id="emptyCartWarning" class="alert alert-info">{l s='Your shopping cart is empty.'}</p>
{*
...
{/if}
*}  
<p class="ordercart-title"><b>{l s='Your shopping cart contains:'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}</span></b></p>
					...
					<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" class="bordercolor product_customization_for_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if} customization alternate_item {if $product@last && $customization@last && !count($gift_products)}last_item{/if}">
						...
						<td class="cart_quantity">
							{if isset($cannotModify) AND $cannotModify == 1}
								<span class="f_left">{if $quantityDisplayed == 0 AND isset($customizedDatas.$productId.$productAttributeId)}{$customizedDatas.$productId.$productAttributeId|@count}{else}{$product.cart_quantity-$quantityDisplayed}{/if}</span>
							{else}
	...
	{if sizeof($discounts)}
		...
		{foreach $discounts as $discount}
			<tr class="cart_discount bordercolor {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
				<td class="cart_discount_name"><h2>{$discount.name}</h2></td>
              ...
              <div class="f_left">1</div>		
                {if strlen($discount.code)}
                			<div class="div_cart_quantity_delete">
                <a href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_discount}" class="price_discount_delete" title="{l s='Delete'}"><i class="icon-trash"></i></a>
                </div>
                {/if}
               </div>
            ...
            <div id="cart_voucher" class="table_block">
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
                        ...
                        <div class="titled_box">
							<h2 class="title_offers"><span>{l s='Take advantage of our offers:'}</span></h2>
                        </div>
							<div id="display_cart_vouchers">
							{foreach $displayVouchers as $voucher}
								<span onclick="$('#discount_name').val('{$voucher.code}');return false;" class="voucher_name">{$voucher.name}</span> - {$voucher.description} <br />
							{/foreach}
							</div>
						{/if}
						</fieldset>
...
<p class="cart_navigation  clearfix inner-top">
	{if !$opc}
		<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" class="exclusive btn btn-default standard-checkout" title="{l s='Next'}">{l s='Next'} &raquo;</a>
		{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
			<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1" class="multishipping-button multishipping-checkout exclusive btn btn-default" title="{l s='Next'}">{l s='Next'} &raquo;</a>
		{/if}
	{/if}
	<a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="button_large btn btn-default" title="{l s='Continue shopping'}">&laquo; {l s='Continue shopping'}</a>
</p>
