...
<form action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
	<div class="titled_box">
        ...
        <input  type="submit" value="{l s='Reorder'}" name="submitReorder" class="btn btn-success" />
	</div>
</form>

<div class="info-order">
{if $carrier->id}<p class="tit-ord"><strong>{l s='Carrier'}</strong> {if $carrier->name == "0"}{$shop_name|escape:'htmlall':'UTF-8'}{else}{$carrier->name|escape:'htmlall':'UTF-8'}{/if}</p>{/if}
<p class="tit-ord"><strong>{l s='Payment method'}</strong> <span class="color-myaccount">{$order->payment|escape:'htmlall':'UTF-8'}</span></p>
{if $invoice AND $invoiceAllowed}
<p>
	<a class="btn btn-success" target="_blank" href="{$link->getPageLink('pdf-invoice', true)}?id_order={$order->id|intval}{if $is_guest}&secure_key={$order->secure_key}{/if}">{l s='Download your invoice as a PDF file.'}</a>
</p>
{/if}
{if $order->recyclable}
<p  class="tit-ord"><img src="{$img_dir}icon/recyclable.png" alt="" class="icon" />&nbsp;{l s='You have given permission to receive your order in recycled packaging.'}</p>
{/if}
{if $order->gift}
...
<div class="titled_box"> 
<h2><span>{l s='Follow your order\'s status step-by-step'}</span></h2>
</div>
...
<div id="order-detail-content" class="table_block">
	<table id="order-detail-content-table" class="std table table-bordered shop_table footable table-hover">
		<thead>
			<tr>
            	{if $return_allowed}<th  class="first_item  checkbox_type"><input type="checkbox" /></th>{/if}
                <th data-hide="phone,tablet" class="{if $return_allowed}item{else}first_item{/if}">{l s='Reference'}</th>
            	<th  data-class="expand" class="item">{l s='Product'}</th>
				<th class="item">{l s='Quantity'}</th>
				{if $order->hasProductReturned()}
					<th data-hide="phone,tablet" class="item">{l s='Returned'}</th>
				{/if}
				<th data-hide="phone,tablet" class="item">{l s='Unit price'}</th>
				<th data-hide="phone,tablet" class="last_item">{l s='Total price'}</th>
			</tr>
		</thead>
		<tfoot>
			{if $priceDisplay && $use_tax}
				<tr class="item">
					<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
						<strong>{l s='Total products (tax excl.)'} </strong><span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithoutTaxes() currency=$currency}</span>
					</td>
				</tr>
			{/if}
			<tr class="item">
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					<strong>{l s='Total products'} {if $use_tax}{l s='(tax incl.)'}{/if}: </strong> <span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithTaxes() currency=$currency}</span>
				</td>
			</tr>
			{if $order->total_discounts > 0}
			<tr class="item">
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					<strong>{l s='Total vouchers:'} </strong> <span class="price-discount">{displayWtPriceWithCurrency price=$order->total_discounts currency=$currency convert=1}</span>
				</td>
			</tr>
			{/if}
			...
			<tr class="item">
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					<strong>{l s='Total gift wrapping cost:'} </strong> <span class="price-wrapping">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
				</td>
			</tr>
			{/if}
			<tr class="item">
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					<strong>{l s='Total shipping'} {if $use_tax}{l s='(tax incl.)'}{/if}: </strong> <span class="price">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
				</td>
			</tr>
			<tr class="totalprice item">
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					<strong>{l s='Total'}</strong> <span class="price">{displayWtPriceWithCurrency price=$order->total_paid currency=$currency}</span>
				</td>
			</tr>
		</tfoot>
		<tbody>
			...
			{if !isset($product.deleted)}
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td><input style="display:none;" class="order_qte_input" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$productQuantity|intval}</span></label></td>
						{if $order->hasProductReturned()}
							<td>
								{$product['qty_returned']}
							</td>
...
{if $return_allowed}
	<div id="returnOrderMessage" class="titled_box">
		<h2><span>{l s='Merchandise return'}</span></h2>
		<p class="textarea">
        	<label>{l s='If you wish to return one or more products, please mark the corresponding boxes and provide an explanation for the return. Then click the button below.'}</label>
			<textarea class="form-control" cols="67" rows="3" name="returnText"></textarea>
		</p>
		<p class="submit">
			<input type="submit" value="{l s='Make an RMA slip'}" name="submitReturnMerchandise" class="btn btn-default" />
...
{if $order->getShipping()|count > 0}
	<table id="shipping-table" class="table table-bordered shop_table footable table-hover">
		<thead>
			<tr>
				<th data-class="expand"  class="first_item">{l s='Date'}</th>
				<th data-hide="phone,tablet" class="item">{l s='Carrier'}</th>
				<th data-hide="phone,tablet" class="item">{l s='Weight'}</th>
				<th data-hide="phone,tablet" class="item">{l s='Shipping cost'}</th>
				<th data-hide="phone,tablet" class="last_item">{l s='Tracking number'}</th>
			</tr>
		</thead>
		<tbody>
...
{if !$is_guest}
	...
	{/if}
	<form action="{$link->getPageLink('order-detail', true)}" method="post" class="std titled_box" id="sendOrderMessage">
		<h2><span>{l s='Add a message'}</span></h2>
...
{else}
<p><i class="icon-info-sign"></i>&nbsp;{l s='You cannot return merchandise with a guest account'}</p>
{/if}
{/if}