...
<h1><span>{l s='Credit slips'}</span></h1>
...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="table table-hover table-bordered footable">
		<thead>
			<tr>
				<th data-class="expand" class="first_item">{l s='Credit slip'}</th>
				<th data-hide="phone" class="item">{l s='Order'}</th>
				<th data-hide="phone,tablet" class="item">{l s='Date issued'}</th>
				<th data-hide="phone,tablet" class="last_item">{l s='View credit slip'}</th>
			</tr>
		</thead>
		<tbody>
	...
	{else}
		<p class="alert alert-info">{l s='You have not received any credit slips.'}</p>
	{/if}
</div>
