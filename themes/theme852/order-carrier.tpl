...
{/if}
{if !$opc}
	<h1><span>{l s='Shipping'}</span></h1>
{else}
	<h1><span>2</span> {l s='Delivery methods'}</h1>
{/if}
...
{if !$opc}
	...
	<form id="form" action="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")|escape:'html'}" method="post" onsubmit="return acceptCGV();" class="titled_box">
{else}
...
{else}
	<div class="titled_box">
		<h2 class="carrier_title"><span>{l s='Choose your delivery method'}</span></h2>
	</div>
	...
	{if isset($isVirtualCart) && $isVirtualCart}
		<p class="alert alert-info">{l s='No carrier is needed for this order.'}</p>
	{else}
			...
			{foreach $option_list as $key => $option}
				<div class="delivery_option {if ($option@index % 2)}alternate_{/if}item">
					<label for="delivery_option_{$id_address}_{$option@index}">
						...
						<table class="delivery_option_carrier table {if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key}selected{/if} {if $option.unique_carrier}not-displayable{/if}">
							{foreach $option.carrier_list as $carrier}
							<tr>
								{if !$option.unique_carrier}
								<td class="first_item">
	...
	<div style="display: none;" id="extra_carrier"></div>
		...
		{if $giftAllowed}
        <div class="titled_box">
		<h2 class="gift_title"><span>{l s='Gift'}</span></h2>
		</div>
        <p class="checkbox-inline">
...
{if $conditions AND $cms_id}
	<div class="titled_box">
		<h2><span>{l s='Terms of service'}</span></h2>
	</div>
    <p class="checkbox-inline">
...
{if !$opc}
	<p class="cart_navigation clearfix submit">
		...
		{if !$is_guest}
			{if $back}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&back={$back}&multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Previous'}" class="button btn btn-default">&laquo; {l s='Previous'}</a>
			{else}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Previous'}" class="button btn btn-default">&laquo; {l s='Previous'}</a>
			{/if}
		{else}
				<a href="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")|escape:'html'}" title="{l s='Previous'}" class="button btn btn-default">&laquo; {l s='Previous'}</a>
		{/if}
		{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
			<input type="submit" name="processCarrier" value="{l s='Next'} &raquo;" class="exclusive btn btn-default" />
		{/if}
	</p>
</form>
{else}
	<div class="titled_box">
		<h2><span>{l s='Leave a message'}</span></h2>
    </div>
	<div class="form-group">
		<label>{l s='If you would like to add a comment about your order, please write it in the field below.'}</label>
		<textarea class="form-control" cols="120" rows="3" name="message" id="message">{if isset($oldMessage)}{$oldMessage|escape:'htmlall':'UTF-8'}{/if}</textarea>
	</div>
</div>
{/if}
</div>
