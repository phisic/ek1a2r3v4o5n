...
<h1><span>{l s='My addresses'}</span></h1>
...
<div class="addresses titled_box">
	<h2><span>{l s='Your addresses are listed below.'}</span></h2>
	...
	{foreach from=$multipleAddresses item=address name=myLoop}
    	<div class="col-xs-12 col-sm-6 col-md-4">
			...
			{/foreach}
			<li class="address_update"><a class="btn btn-success" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html'}" title="{l s='Update'}">{l s='Update'}</a></li>
			<li class="address_delete"><a class="btn btn-danger" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html'}" onclick="return confirm('{l s='Are you sure? js=1'}');" title="{l s='Delete'}">{l s='Delete'}</a></li>
		</ul>
        </div>
	{/foreach}
	</div>
...
{else}
    ...
    <i class="icon-info-sign"></i>{l s='No addresses are available.'}&nbsp;<a href="{$link->getPageLink('address', true)|escape:'html'}">{l s='Add a new address'}</a></p>
{/if}

<div class="clear address_add"><a href="{$link->getPageLink('address', true)|escape:'html'}" title="{l s='Add an address'}" class="button_large btn btn-default">{l s='Add an address'}</a></div>
