...
<section id="suppliers_block_left" class="block blocksupplier column_box">
	<h4 class="title_block">{if $display_link_supplier}<span>{/if}{l s='Suppliers' mod='blocksupplier'}{if $display_link_supplier}</span>{/if}<i class="column_icon_toggle icon-plus-sign"></i></h4>
...
{if $suppliers}
	...
	{foreach from=$suppliers item=supplier name=supplier_list}
		...
		<li class="{if $smarty.foreach.supplier_list.last}last_item{elseif $smarty.foreach.supplier_list.first}first_item{else}item{/if}">
			<a href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'html'}" title="{l s='More about' mod='blocksupplier'} {$supplier.name}"><i class="icon-ok"></i>{$supplier.name|escape:'htmlall':'UTF-8'}</a>
		</li>
		{/if}
	{/foreach}
	</ul>
