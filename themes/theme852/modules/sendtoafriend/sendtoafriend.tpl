...
<div id="sendfriendpage">
	...
	{if isset($smarty.get.submited)}
		<p class="alert succes"><button class="close" data-dismiss="alert" type="button">×</button> {l s='Your email has been sent successfully' mod='sendtoafriend'}</p>
	{else}
			<form method="post" action="{$request_uri}" class="std">
				<fieldset>
					...
					<div id="send_friend_form_content">
						...
						<p class="submit">
							<input type="submit" name="submitAddtoafriend" value="{l s='send' mod='sendtoafriend'}" class="button btn btn-default" />
						</p>
					</div>
				</fieldset>
			</form>
