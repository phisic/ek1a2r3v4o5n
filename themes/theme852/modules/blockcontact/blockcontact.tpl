...
<section id="contact_block" class="block column_box">
	...
	<div class="block_content clearfix toggle_content">
			...
			{if $telnumber != ''}<p><span class="label">{l s='Phone:' mod='blockcontact'}</span><i class="icon-phone"></i> {$telnumber|escape:'htmlall':'UTF-8'}</p>{/if}
			{if $email != ''}<b><a class="links" href="mailto:{$email|escape:'htmlall':'UTF-8'}">{l s='Contact our hotline' mod='blockcontact'}</a></b>{/if}
	</div>
</section>
