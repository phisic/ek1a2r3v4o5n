...
<h1><span>{l s='Referral program' mod='referralprogram'}</span></h1>
...
<div class="sheets tab-content">
	...
	<div id="idTab1" class="tab-pane active">
		...
		{if $canSendInvitations}
			...
			<form method="post" action="{$link->getModuleLink('referralprogram', 'program', [], true)|escape:'html'}" class="std">
				...
				<p class="submit">
					<input type="submit" id="submitSponsorFriends" name="submitSponsorFriends" class="button_large btn btn-default" value="{l s='Validate' mod='referralprogram'}" />
				</p>
			</form>
		{else}
	...
	{if $pendingFriends AND $pendingFriends|@count > 0}
		...
		<form method="post" action="{$link->getModuleLink('referralprogram', 'program', [], true)|escape:'html'}" class="std">
			...
			<p class="submit">
				<input type="submit" value="{l s='Remind my friend(s)' mod='referralprogram'}" name="revive" id="revive" class="button_large btn btn-default" />
			</p>
		</form>
		{else}
			<p class="alert alert-info">{l s='You have not sponsored any friends.' mod='referralprogram'}</p>
		{/if}
	</div>
...
<ul class="footer_links clearfix">
	<li><a href="{$link->getPageLink('my-account', true)|escape:'html'}" title="{l s='Back to Your Account' mod='referralprogram'}" rel="nofollow"><i class="icon-user"></i> {l s='Back to Your Account' mod='referralprogram'}</a></li>
	<li class="f_right"><a href="{$base_dir}" title="{l s='Home' mod='referralprogram'}"><i class="icon-home"></i> {l s='Home' mod='referralprogram'}</a></li>
</ul>
