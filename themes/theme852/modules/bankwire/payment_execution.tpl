...
{if $nbProducts <= 0}
	<p class="alert alert-info">{l s='Your shopping cart is empty.' mod='bankwire'}</p>
{else}
...
<i class="icon-angle-right"></i> 
	{if $currencies|@count > 1}
		...
		<select id="currency_payement" name="currency_payement" onchange="setCurrency($('#currency_payement').val());" class="form-control">
...
<p class="cart_navigation clearfix">
	<input type="submit" name="submit" value="{l s='Place my order' mod='bankwire'}" class="exclusive_large btn btn-default" />
	<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html'}" class="button_large btn btn-default">{l s='Other payment methods' mod='bankwire'}</a>
</p>
</form>
{/if}
