...
<section id="links_block_left" class="block column_box">
	...
	{foreach from=$blocklink_links item=blocklink_link}
		{if isset($blocklink_link.$lang)} 
			<li><a href="{$blocklink_link.url|escape}"{if $blocklink_link.newWindow} title="{$blocklink_link.$lang|escape}" onclick="window.open(this.href);return false;"{/if}><i class="icon-ok"></i>{$blocklink_link.$lang|escape}</a></li>
		{/if}
	{/foreach}
	</ul>
</section>
