...
<h1><span>{l s='My loyalty points' mod='loyalty'}</span></h1>

{if $orders}
<script type="text/javascript">
    $(function() {
      $('#block-history table').footable()
	  breakpoints: {
 		phone:480
		tablet:1170
		}
...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="footable table table-bordered table-hover">
		<thead>
			<tr>
				<th data-class="expand" class="first_item">{l s='Order' mod='loyalty'}</th>
				<th data-hide="phone" class="item">{l s='Date' mod='loyalty'}</th>
				<th class="item">{l s='Points' mod='loyalty'}</th>
				<th data-hide="phone" class="last_item">{l s='Points Status' mod='loyalty'}</th>
			</tr>
		</thead>
		<tfoot>
			<tr class="alternate_item">
				<td colspan="4" class="history_method bold">{l s='Total points available:' mod='loyalty'} {$totalPoints|intval}</td>
			</tr>
		</tfoot>
		<tbody>
	...
	{else}
		<p class="alert alert-info">{l s='You have not placed any orders.'}</p>
	{/if}
</div>
<div class="clearfix">
    <div id="pagination" class="pagination">
        {if $nbpagination < $orders|@count}
            ...
            {if $page != 1}
                ...
                <li id="pagination_previous"><a href="{summarypaginationlink p=$p_previous n=$nbpagination}" title="{l s='Previous' mod='loyalty'}" rel="nofollow">&laquo;&nbsp;{l s='Previous' mod='loyalty'}</a></li>
            {else}
...
<p class="align_center top_indent">
	<a href="{$link->getModuleLink('loyalty', 'default', ['process' => 'transformpoints'])|escape:'html'}" onclick="return confirm('{l s='Are you sure you want to transform your points into vouchers?' mod='loyalty' js=1}');" class="btn btn-default">{l s='Transform my points into a voucher of' mod='loyalty'} <span class="price">{convertPrice price=$voucher}</span>.</a>
</p>
{/if}
</p>


<h1><span>{l s='My vouchers from loyalty points' mod='loyalty'}</span></h2>
...
<div class="block-center" id="block-history">
	<table id="order-list" class="footable table table-bordered table-hover">
		<thead>
			<tr>
				<th data-class="expand" class="first_item">{l s='Created' mod='loyalty'}</th>
				<th class="item">{l s='Value' mod='loyalty'}</th>
				<th class="item">{l s='Code' mod='loyalty'}</th>
				<th data-hide="phone,tablet" class="item">{l s='Valid from' mod='loyalty'}</th>
				<th data-hide="phone,tablet" class="item">{l s='Valid until' mod='loyalty'}</th>
				<th data-hide="phone,tablet" class="item">{l s='Status' mod='loyalty'}</th>
				<th data-hide="phone,tablet" class="last_item">{l s='Details' mod='loyalty'}</th>
			</tr>
		</thead>
		<tbody>
...
{else}
<p class="alert alert-info">{l s='No vouchers yet.' mod='loyalty'}</p>
{/if}
{else}
<p class="alert alert-info">{l s='No reward points yet.' mod='loyalty'}</p>
{/if}

<ul class="footer_links clearfix">
	<li><a href="{$link->getPageLink('my-account', true)|escape:'html'}" rel="nofollow"><i class="icon-user"></i> {l s='Back to Your Account' mod='loyalty'}</a></li>
	<li class="f_right"><a href="{$base_dir}"><i class="icon-home"></i> {l s='Home' mod='loyalty'}</a></li>
</ul>
