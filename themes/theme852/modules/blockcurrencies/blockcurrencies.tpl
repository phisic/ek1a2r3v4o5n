...
<span class="block-name">{l s='Currency' mod='blockcurrencies'}</span>
		...
		<ul id="first-currencies" class="currencies_ul list_header">
			{foreach from=$currencies key=k item=f_currency}
				<li {if $cookie->id_currency == $f_currency.id_currency}class="selected"{/if}>
                    ...
                    {else}
                        <a href="javascript:setCurrency({$f_currency.id_currency});" title="{$f_currency.name}" rel="nofollow"><span>{$f_currency.sign}</span>{$f_currency.name}</a>			
                    {/if}
				</li>
			{/foreach}
		</ul>
