...
<section id="layered_block_left" class="column_box block">
	<h3 class="toggle"><span>{l s='Catalog' mod='blocklayered'}</span></h3>
			...
			<div>
								...
								{if $filter.type == $filter_type && isset($filter.values)}
									{if isset($filter.slider)}
										{if $smarty.foreach.f_values.first}
											<li>
												<a href="#" rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered'}"><i class="icon-trash"></i></a>
									...
									{else}
										{foreach from=$filter.values key=id_value item=value}
											{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
												<li>
													<a href="#" rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}"><i class="icon-trash"></i></a>
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						{if isset($filter.slider)}
							<div class="layered_{$filter.type} titled_box" style="display: none;">
						{else}
							<div class="titled_box">
						{/if}
                        <h2 class="clearfix">
							<span class="layered_subtitle">{$filter.name|escape:html:'UTF-8'}</span>
							<span class="layered_close"><a href="#" rel="ul_layered_{$filter.type}_{$filter.id_key}"><i class="icon-minus-sign"></i></a></span>
						</h2>
							...
							{else}
								...
								<li class="nomargin row">
                                	<div class="form-group clearfix col-xs-6 col-sm-12">
										<label class="col-xs-2 control-label">{l s='From' mod='blocklayered'}</label>
                                        <div class="col-xs-9">
                                        ...
                                        <span class="layered_{$filter.type}_range_unit col-xs-1 control-label">{$filter.unit}</span>
									</div>
                                    <div class="form-group clearfix col-xs-6 col-sm-12">
                                        <label class="col-xs-2 control-label">{l s='to' mod='blocklayered'}</label>
                                        <div class="col-xs-9">
								...
								{foreach from=$filter.list_of_values  item=values}
									<li class="nomargin link_type {if $filter.values[1] == $values[1] && $filter.values[0] == $values[0]}layered_list_selected{/if} layered_list" onclick="$('#layered_{$filter.type}_range_min').val({$values[0]});$('#layered_{$filter.type}_range_max').val({$values[1]});reloadContent();">
										<i class="icon-caret-right"></i> {l s='From' mod='blocklayered'} {$values[0]} {$filter.unit} {l s='to' mod='blocklayered'} {$values[1]} {$filter.unit}
									</li>
								{/foreach}
	...
	<div id="layered_ajax_loader" style="display: none;">
		<p class="loader_position"><img src="{$img_dir}loader-filter.gif" alt="" /></p>
	</div>
	</section>
{else}
