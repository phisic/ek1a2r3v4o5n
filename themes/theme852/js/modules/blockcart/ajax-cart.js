						...
						$('#'+removedProductId).addClass('strike').fadeTo('slow', 0, function(){
							$(this).slideUp('slow', function(){
								...
								if($('#cart_block dl.products dt').length == 0)
								{
									...
									$('#header .cart-prices, #header #cart-buttons, #header #cart-price-precisions').hide(); // hide buttons and prices if no more products
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('#cart_block dl.products').length == 0)
				{
					...
					$('#header .cart-prices, #header #cart-buttons, #header #cart-price-precisions').show(); // show buttons and prices if product added
				}
