...
{if isset($p) AND $p}
	...
	{if $nb_products > $products_per_page}
		<form action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get" class="nbrItemPage nbr_page">
				...
				{if is_array($requestNb)}
					...
					{/foreach}
				{/if}
                <select name="n" id="nb_item{if isset($paginationId)}_{$paginationId}{/if}" class="js-nb_item nb_item" onchange="this.form.submit();">
