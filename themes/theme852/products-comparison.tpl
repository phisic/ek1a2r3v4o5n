	...
	{foreach from=$products item=product name=for_products}
		...
		<td class="ajax_block_product">
			...
			<div class="row-compare-button">
				<a class="button btn btn-default" href="{$product->getLink()}" title="{l s='View'}">{l s='View'}</a>
				...
				{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
					{if ($product->quantity > 0 OR $product->allow_oosp)}
							<a class="exclusive ajax_add_to_cart_button btn btn-default btn_add_cart" rel="ajax_id_product_{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html'}" title="{l s='Add to cart'}"><span>{l s='Add to cart'}</span></a>
					{else}
...
{else}
	<p class="alert alert-info">{l s='There are no products selected for comparison.'}</p>
{/if}
