...
<h1><span>{l s='Return Merchandise Authorization (RMA)'}</span></h1>
{if isset($errorQuantity) && $errorQuantity}<p class="error alert alert-danger">{l s='You do not have enough products to request an additional merchandise return.'}</p>{/if}
{if isset($errorMsg) && $errorMsg}
    ...
    <div class="titled_box">
		...
		<form method="POST"  id="returnOrderMessage">
			...
			<input class="btn btn-default" type="submit" name="submitReturnMerchandise" value="{l s='Make an RMA slip'}"/>
		</form>
	</div>
{/if}
{if isset($errorDetail1) && $errorDetail1}<p class="error alert alert-danger">{l s='Please check at least one product you would like to return.'}</p>{/if}
{if isset($errorDetail2) && $errorDetail2}<p class="error alert alert-danger">{l s='For each product you wish to add, please specify the desired quantity.'}</p>{/if}
{if isset($errorNotReturnable) && $errorNotReturnable}<p class="error alert alert-danger">{l s='This order cannot be returned.'}</p>{/if}
...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="std table table-bordered table-hover footable">
		<thead>
			<tr>
				...
				<th data-hide="phone,tablet" class="item">{l s='Package status'}</th>
	...
	{else}
		<p class="alert alert-info">{l s='You have no merchandise return authorizations.'}</p>
	{/if}
</div>
