...
{if !$opc}<h1><span>{l s='Addresses'}</span></h1>{else}<h1><span>1 {l s='Addresses'}</span></h1>{/if}
...
{if !$opc}
	...
	{if !$multi_shipping && {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="button_multishipping_mode titled_box" id="multishipping_mode_box">
			<h2><span>{l s='Multi-shipping'}</span></h2>
			<div class="description">
				<a class="button btn btn-default" href="{$link->getPageLink('order', true, NULL, 'step=1&multi-shipping=1')|escape:'html'}"/>
...
{else}
	{if {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="address-form-multishipping hidden-xs">
			<div class="button_multishipping_mode" id="multishipping_mode_box">
				<h4 class="title">{l s='Multi-shipping'}</h4>
				<div class="description">
				...
				<div class="description_off">
					<a href="{$link->getPageLink('order-opc', true, NULL, 'ajax=1&multi-shipping=1&method=multishipping')|escape:'html'}" id="link_multishipping_form" class="button btn btn-default" title="{l s='Choose the delivery address(es)'}">
...
{/if}
	<div class="addresses clearfix">
		<p class="address_delivery form-group">
		...
		<p class="checkbox-inline addressesAreEquals" {if $cart->isVirtualCart()}style="display:none;"{/if}>
		...
		{if $addresses|@count > 1}
			...
			{else}
				<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large btn btn-default">{l s='Add a new address'}</a>
			{/if}
		</p>
		<div class="row bottom_indent">
        	<div class="col-xs-12 col-sm-6">
                <ul class="address item clearfix" id="address_delivery" {if $cart->isVirtualCart()}style="display:none;"{/if}>
                </ul>
            </div>
            <div class="col-xs-12 col-sm-6">
				<ul class="address alternate_item {if $cart->isVirtualCart()}full_width{/if} clearfix" id="address_invoice">
				</ul>
            </div>
		</div>
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large btn btn-default">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
		<div id="ordermsg" class="clearfix form-group">
			<label>{l s='If you would like to add a comment about your order, please write it in the field below.'}</label>
			<textarea class="form-control" cols="60" rows="3" name="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea>
		</div>
		{/if}
	</div>
{if !$opc}
	<p class="cart_navigation submit clearfix">
		...
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html'}" title="{l s='Previous'}" class="button btn btn-default">&laquo; {l s='Previous'}</a>
		<input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="exclusive btn btn-default" />
	</p>
</form>
{else}
</div>
