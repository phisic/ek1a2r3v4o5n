	...
	{else}
		<form method="post" action="{$action|escape:'htmlall':'UTF-8'}#guestToCustomer" class="std">
			<fieldset class="description_box">
				...
				<p class="center"><input type="submit" class="exclusive_large btn btn-default" name="submitTransformGuestToCustomer" value="{l s='Send'}" /></p>
			</fieldset>
		</form>
	{/if}
{else}
	...
	{if isset($show_login_link) && $show_login_link}
		<p><a href="{$link->getPageLink('my-account', true)|escape:'html'}"><i class="icon-user"></i> {l s='Click here to login to your customer account.'}</a><br /><br /></p>
	{/if}
	<form method="post" action="{$action|escape:'htmlall':'UTF-8'}" class="std">
		<fieldset class="description_box">
			...
			<p class="form-group">
				...
				<i><small>{l s='For example: QIIXJXNUI or QIIXJXNUI#1'}</small></i>
			</p>
			...
			<p class="center"><input type="submit" class="button btn btn-default" name="submitGuestTracking" value="{l s='Send'}" /></p>
		</fieldset>
	</form>
{/if}