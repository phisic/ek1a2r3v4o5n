...
<h1><span>{l s='Suppliers:'}<strong>{strip}{if $nbSuppliers == 0}{l s='There are no suppliers.'}{else}{if $nbSuppliers == 1}{l s='There is %d supplier.' sprintf=$nbSuppliers}{else}{l s='There are %d suppliers.' sprintf=$nbSuppliers}{/if}{/if}{/strip}</strong></span></h1>
		...
		<li class="shop_box clearfix {if $supplier@first}first_item{elseif $supplier@last}last_item{else}item{/if}">
        	<div class="col-xs-12 col-sm-8 col-lg-9 border_sep">
            	<div class="row">
                    ...
                    {/if}
                        <img src="{$img_sup_dir}{$supplier.image|escape:'htmlall':'UTF-8'}-medium_default.jpg" alt=""  />
                    {if $supplier.nb_products > 0}
                    </a>
                    {/if}
                    </div>
                    ...
                    {/if}
                             <span>{$supplier.description|escape:'html':'UTF-8'|strip_tags|truncate:160:'...'}</span>
                    {if $supplier.nb_products > 0}
                    </p>
                    {/if}
			...
			{if $supplier.nb_products > 0}
				<a class="button btn btn-default" href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'htmlall':'UTF-8'}">{l s='View products'}</a>
			{/if}
			</div>
		</li>
	{/foreach}
